...
class Qwen2VLTemplate(Template):
    ...
    i_must_reply_now_token_ids = [-100, 40, 1969, 9851, 1431, 624]      # 'I must reply now.\n' at the start of assistant content
    version = 'v2'

    def _preprocess_inputs(self, *args, **kwargs):
        self.h5_cache = dict()
        res = super()._preprocess_inputs(*args, **kwargs)
        del self.h5_cache
        return res

    def _load_image(self, image, load_images):
        # Store multiple frames of a video in a single hdf5 file to avoid fragmented disk access when reading large numbers of images
        if load_images and isinstance(image, dict) and ':::' in image.get('path', ''):
            h5_filename, image_index = image['path'].split(':::')
            image_index = int(image_index)
            if h5_filename not in self.h5_cache:
                import h5py
                with h5py.File(h5_filename, 'r') as f:
                    self.h5_cache[h5_filename] = np.array(f['images'])
            np_image = self.h5_cache[h5_filename][image_index]
            image = Image.fromarray(np_image)
            return image
        else:
            return super()._load_image(image, load_images)
    ...
    def replace_tag(self, media_type: Literal['image', 'video', 'audio'], index: int,
        ...
        if media_type == 'image':
            ...
            # MMDuet2 modify
            if isinstance(inputs.videos[index], dict):
                if '[' in inputs.videos[index]['video'] and ']' in inputs.videos[index]['video']:        # is a list of images
                    inputs.videos[index]['video'] = json.loads(inputs.videos[index]['video'])
                inputs.videos[index] = fetch_video(inputs.videos[index])
            else:
                raise ValueError(f"element in videos must be filename or dict, but get {inputs.videos[index]}")
    ...
    # MMDuet2 modify
    def remove_subsequence_loss_from_label(self, labels: List[int], target_subseq: List[int]):
        i = 0
        target_len = len(target_subseq)
        while i < len(labels) - target_len:
            if labels[i: i+target_len] == target_subseq:
                labels[i:i + target_len] = [-100] * target_len
                i += target_len
            else:
                i += 1
        return labels
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        # PROACTIVE MODEL modify: 
        # delete "I must answer now" token_ids from labels, as we do not want to calculate loss for them
        labels = self.remove_subsequence_loss_from_label(labels, self.i_must_reply_now_token_ids)
