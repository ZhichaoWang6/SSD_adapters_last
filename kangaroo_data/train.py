...
for epoch in range(args.start, args.start + 1):
    ...
    # 2) 额外保存 HF adapter 格式（config.json + pytorch_model.bin）
    if accelerator.is_local_main_process:
        hf_dir = os.path.join(args.cpdir, f"hf_adapter_epoch_{epoch}")
        os.makedirs(hf_dir, exist_ok=True)

        # 用 base 模型生成 LlamaConfig（确保 hidden_size/vocab_size 等与 base 一致）
        llama_cfg = LlamaConfig.from_pretrained(args.basepath)
        llama_cfg.kangaroo_exit_layer = int(args.exit_layer)
        llama_cfg.kangaroo_residual = train_config["residual"]
        llama_cfg.kangaroo_act = train_config["act"]
        llama_cfg.kangaroo_max_len = train_config["max_len"]
        llama_cfg.save_pretrained(hf_dir)  # 写 config.json

        # 导出 adapter 权重：unwrap 后保存 state_dict
        unwrapped = accelerator.unwrap_model(model).cpu()
        torch.save(unwrapped.state_dict(), os.path.join(hf_dir, "pytorch_model.bin"))

        print(f"[HF EXPORT] saved to: {hf_dir}")
